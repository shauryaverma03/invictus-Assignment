...
**How to reproduce:** This one only shows up depending on what time of day it is, and where in the world the person using the app actually is. If you open the app in India (or anywhere noticeably ahead of London time) between roughly midnight and 5:30 AM local time, the Date field in "Add expense" defaults to *yesterday's* date instead of today's. The reverse happens for anyone noticeably behind London time, late in the evening — the field defaults to *tomorrow*.

**What is wrong:** The default date was built with `new Date().toISOString().slice(0, 10)`. `toISOString()` doesn't give you the date on the clock in front of you — it always converts to UTC first. Most of the time that's invisible, because most of the day your local date and the UTC date happen to agree. But for a window of hours around midnight, they don't — a country ahead of UTC (like India, five and a half hours ahead) has already turned over to a new calendar day locally while UTC is still finishing the old one, and a country behind UTC hasn't turned over locally yet while UTC already has. I didn't just eyeball this — I ran the exact function against a fixed timestamp of 2:00 AM on September 2nd in India Standard Time, and it returned September 1st. Anyone logging a late-night or early-morning bill — which, on an actual trip, covering a late dinner, a red-eye flight, drinks after midnight, is exactly when a lot of real expenses get logged — would have it silently misdated by a full day unless they noticed and manually corrected the date field before saving. That misdated expense would then also sort to the wrong place in "Newest first."

**What I changed:** Rebuilt the default date from the browser's local calendar fields (year, month, day as your own clock reports them) instead of converting through UTC first. I re-ran the same fixed-timestamp check afterward — 2:00 AM IST on September 2nd now correctly returns September 2nd.
